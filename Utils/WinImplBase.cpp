...
LRESULT WindowImplBase::ResponseDefaultKeyEvent(WPARAM wParam, LPARAM lParam, bool& bHandled)
{
	if (wParam == VK_RETURN)
	{
		return FALSE;
...
LRESULT WindowImplBase::OnSysCommand(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	...
	if (::IsZoomed(m_hWnd) != bZoomed)
	{
		if (!bZoomed)
		{
			CControlUI* pControl = m_PaintManager.FindControl(kMaxButtonControlName, false);
			if (pControl) pControl->SetVisible(false);
			pControl = m_PaintManager.FindControl(kRestoreButtonControlName, false);
...
void WindowImplBase::OnClick(TNotifyUI& msg)
{
    ...
    if (sCtrlName == kCloseButtonControlName)
	{
		Close(IDCLOSE);
		return; 
	}
    else if (sCtrlName == kMinButtonControlName)
	{ 
		SendMessage(WM_SYSCOMMAND, SC_MINIMIZE, 0); 
		return; 
	}
    else if (sCtrlName == kMaxButtonControlName)
	{ 
		SendMessage(WM_SYSCOMMAND, SC_MAXIMIZE, 0); 
		return; 
	}
    else if (sCtrlName == kRestoreButtonControlName)
	{ 
...
BOOL WindowImplBase::SetTimer(UINT nTimerID, UINT uElapse)
{
	return ::SetTimer(this->GetHWND(), nTimerID, uElapse, NULL);
}

void WindowImplBase::KillTimer(UINT nTimerID)
{
	::KillTimer(this->GetHWND(), nTimerID);
}
