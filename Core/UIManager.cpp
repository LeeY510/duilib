...
void CPaintManagerUI::ReapObjects(CControlUI* pControl)
{
    ...
    if( !sName.IsEmpty() ) {
        if( pControl == FindControl(sName, false) ) m_mNameHash.Remove(sName);
    }
...
const TImageInfo* CPaintManagerUI::GetShareImage(LPCTSTR bitmap)
{
    TImageInfo* data = static_cast<TImageInfo*>(m_mShareImageHash.Find(bitmap));
    return data;
}
...
const TImageInfo* CPaintManagerUI::GetImageEx(LPCTSTR bitmap, LPCTSTR type, DWORD mask)
{
    ...
    if (!data)
    {
        data = static_cast<TImageInfo*>(m_mShareImageHash.Find(bitmap));
    }
...
const TImageInfo* CPaintManagerUI::GetGdiplusImage(LPCTSTR bitmap)
{
    TImageInfo* data = static_cast<TImageInfo*>(m_mGdiplusImageHash.Find(bitmap));
    return data;
}

const TImageInfo* CPaintManagerUI::GetShareGdiplusImage(LPCTSTR bitmap)
{
    TImageInfo* data = static_cast<TImageInfo*>(m_mGdiplusShareImageHash.Find(bitmap));
    return data;
}

const TImageInfo* CPaintManagerUI::GetGdiplusImageEx(LPCTSTR bitmap)
{
    TImageInfo* data = static_cast<TImageInfo*>(m_mGdiplusImageHash.Find(bitmap));
    if (NULL == data)
    {
        data = static_cast<TImageInfo*>(m_mGdiplusShareImageHash.Find(bitmap));
    }
    return data;
}
...
const TImageInfo* CPaintManagerUI::AddImage(LPCTSTR bitmap, HBITMAP hBitmap, int iWidth, int iHeight, bool bAlpha, bool bShare)
{
    ...
    if (bShare)
    {
        ret = m_mShareImageHash.Insert(bitmap, data);
    }
    else
    {
        ret = m_mImageHash.Insert(bitmap, data);
    }
...
const TImageInfo* CPaintManagerUI::AddGdiplusImage(LPCTSTR bitmap, LPCTSTR type /*= NULL*/, bool bShare /*= false*/)
{
    TImageInfo* data = NULL;
    if (type != NULL)
    {
    ...
    {
        data = CRenderEngine::LoadGdiplusImage(bitmap);
    }
    ...
    if (bShare)
    {
        ret = m_mGdiplusShareImageHash.Insert(bitmap, data);
    }
    else
    {
        ret = m_mGdiplusImageHash.Insert(bitmap, data);
    }
...
{
#ifdef UNICODE
    std::wstring sImageName = bitmap;
    int iStartPos = sImageName.find(L"file='");
    if (std::wstring::npos != iStartPos)
    {
        int iEndPos = sImageName.find('\'', iStartPos + 6);
        sImageName = sImageName.substr(iStartPos + 6, iEndPos - (iStartPos + 6));
    }    
#else
    std::string sImageName = bitmap;
    int iStartPos = sImageName.find("file='");
    if (std::string::npos != iStartPos)
    {
        int iEndPos = sImageName.find('\'', iStartPos + 6);
        sImageName = sImageName.substr(iStartPos + 6, iEndPos - (iStartPos + 6));
    }
...
#endif
    const TImageInfo* data = GetImage(sImageName.c_str());
    if (!data) return false;

    CRenderEngine::FreeImage(data);

    return m_mImageHash.Remove(sImageName.c_str());    
}

bool CPaintManagerUI::RemoveShareImage(LPCTSTR bitmap)
{
#ifdef UNICODE
    std::wstring sImageName = bitmap;
    int iStartPos = sImageName.find(L"file='");
    ...
    const TImageInfo* data = GetShareImage(sImageName.c_str());
    if (!data) return false;

    CRenderEngine::FreeImage(data);

    return m_mShareImageHash.Remove(sImageName.c_str());
}
...
void CPaintManagerUI::RemoveAllImages()
{
    ...
    for (int i = 0; i < m_mGdiplusImageHash.GetSize(); i++) {
        if (LPCTSTR key = m_mGdiplusImageHash.GetAt(i)) {
            data = static_cast<TImageInfo*>(m_mGdiplusImageHash.Find(key, false));
            if (data) {
                CRenderEngine::FreeImage(data);
            }
        }
    }
    m_mGdiplusImageHash.RemoveAll();
}

void CPaintManagerUI::RemoveAllShareImages()
{
    TImageInfo* data;
    for (int i = 0; i < m_mShareImageHash.GetSize(); i++) {
        if (LPCTSTR key = m_mShareImageHash.GetAt(i)) {
            data = static_cast<TImageInfo*>(m_mShareImageHash.Find(key, false));
            if (data) {
                CRenderEngine::FreeImage(data);
            }
        }
    }
    m_mShareImageHash.RemoveAll();

    for (int i = 0; i < m_mGdiplusShareImageHash.GetSize(); i++) {
        if (LPCTSTR key = m_mGdiplusShareImageHash.GetAt(i)) {
            data = static_cast<TImageInfo*>(m_mGdiplusShareImageHash.Find(key, false));
            if (data) {
                CRenderEngine::FreeImage(data);
            }
        }
    }
    m_mGdiplusShareImageHash.RemoveAll();
}

bool CPaintManagerUI::RemoveGdiplusImage(LPCTSTR bitmap)
{
    const TImageInfo* data = GetGdiplusImage(bitmap);
    if (!data) return false;

    CRenderEngine::FreeImage(data);

    return m_mGdiplusImageHash.Remove(bitmap);
}

bool CPaintManagerUI::RemoveShareGdiplusImage(LPCTSTR bitmap)
{
    const TImageInfo* data = GetShareGdiplusImage(bitmap);
    if (!data) return false;

    CRenderEngine::FreeImage(data);

    return m_mGdiplusShareImageHash.Remove(bitmap);
}
...
CControlUI* CPaintManagerUI::FindControl(LPCTSTR pstrName, bool is_assert/* = true*/) const
{
    ASSERT(m_pRoot);
    CControlUI* pCtrl = static_cast<CControlUI*>(m_mNameHash.Find(pstrName));
    if (is_assert)
    {
        ASSERT(pCtrl);
    }
...
CControlUI* CALLBACK CPaintManagerUI::__FindControlFromPoint(CControlUI* pThis, LPVOID pData)
{
    ...
    if (pThis->IsSpecialCtrl())
    {

        return pThis->PointInRegion(*pPoint) ? pThis : NULL;
    }
