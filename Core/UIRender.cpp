...
bool CRenderEngine::GetImageData(const LPCTSTR pstrImagePath, const LPCTSTR type, LPBYTE* ppData, DWORD& dwSize)
{
    LPBYTE pData = NULL;
    dwSize = 0;
    ...
    do
    {
        ...
        {
            HRSRC hResource = ::FindResource(CPaintManagerUI::GetResourceDll(), pstrImagePath, type);
...
Gdiplus::Bitmap* CRenderEngine::LoadImageMemory(LPVOID pBuf, size_t dwSize)
{
...
TImageInfo* CRenderEngine::LoadGdiplusImage(const LPCTSTR pStrBitmap, LPCTSTR type /*= NULL*/, DWORD mask/* = 0*/)
{
    LPBYTE pData = NULL;
    DWORD dwSize = 0;
    if (!GetImageData(pStrBitmap, type, &pData, dwSize))
    {
        return NULL;
    }

    HGLOBAL hGlobalMem = ::GlobalAlloc(GMEM_MOVEABLE | GMEM_NODISCARD, dwSize);
    BYTE* pMem = (BYTE*)::GlobalLock(hGlobalMem);
    ...
    if (S_OK != ::CreateStreamOnHGlobal(hGlobalMem, TRUE, &pStream))
    {
    ...
    pStream->Release();
    if (NULL == pImage || pImage->GetLastStatus() != Gdiplus::Ok)
    {
        return NULL;
    }

    TImageInfo* data = new TImageInfo;
    data->pGdiplusImage = pImage;
    data->bIsGdiplus = true;
    return data;
}
...
TImageInfo* CRenderEngine::LoadImage(STRINGorID bitmap, LPCTSTR type, DWORD mask)
{
    ...
    if (!GetImageData(bitmap.m_lpstr, type, &pData, dwSize))
    {
        return NULL;
    }
...
HBITMAP CRenderEngine::LoadImage(unsigned char* bytesImg, DWORD bytesLength, int& iWidth, int& iHeight, bool& bAlphaChannel, LPCTSTR type/* = NULL*/, DWORD mask/* = 0*/)
{
	LPBYTE pImage = NULL;
	int x = 1, y = 1, n;
	if (!type)
...
HBITMAP CRenderEngine::LoadBGRImage(unsigned char* bytesImg, DWORD bytesLength, int iWidth, int iHeight)
{
    BITMAPINFO bmi;
    ::ZeroMemory(&bmi, sizeof(BITMAPINFO));
    bmi.bmiHeader.biSize = sizeof(BITMAPINFOHEADER);
    ...
    bmi.bmiHeader.biCompression = BI_RGB;
    bmi.bmiHeader.biSizeImage = bytesLength;
    LPBYTE pDest = NULL;
    HBITMAP hBitmap = ::CreateDIBSection(NULL, &bmi, DIB_RGB_COLORS, (void**)&pDest, NULL, 0);
    if (!hBitmap)
    {
    ...
    memcpy(pDest, bytesImg, bytesLength);
...
{
    if (bitmap->bIsGdiplus)
    {
        if (NULL != bitmap->pGdiplusImage)
        {
            delete bitmap->pGdiplusImage;
        }
    }
    else
    {
