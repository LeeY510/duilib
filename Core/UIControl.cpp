...
void CControlUI::SetBkImage(LPCTSTR pStrImage, unsigned char* bytesImg, DWORD bytesLength, bool bShare)
{
    if (bShare)
    {
        CPaintManagerUI::RemoveShareImage(pStrImage);
    }
    else
    {
    ...
    HBITMAP hBitmap = CRenderEngine::LoadImage(bytesImg, bytesLength, iWidth, iHeight, bAlphaChannel);
    ...
    if (!m_pManager->AddImage(sImageName.c_str(), hBitmap, iWidth, iHeight, bAlphaChannel, bShare))
    {
        return;
    }
...
void CControlUI::SetBGRBkImage(LPCTSTR pStrImage, unsigned char* bytesImg, DWORD bytesLength, int iWidth, int iHeight, bool bShare)
{
...
bool CControlUI::SetBGRImage(LPCTSTR pStrImage, unsigned char* bytesImg, DWORD bytesLength, int iWidth, int iHeight, bool bShare)
{
    if (bShare)
    {
        CPaintManagerUI::RemoveShareImage(pStrImage);
    ...
    bool bAlphaChannel = false;
    HBITMAP hBitmap = CRenderEngine::LoadBGRImage(bytesImg, bytesLength, iWidth, iHeight);
    ...
    if (NULL == m_pManager->AddImage(sImageName.c_str(), hBitmap, iWidth, iHeight, bAlphaChannel, bShare))
    {
        return false;
    }
...
void CControlUI::NeedUpdate(bool bRefresh/*=true*/)
{
    if( !IsVisible() ) return;
    m_bUpdateNeeded = true;
    if (bRefresh)
...
void CControlUI::PaintBorder(HDC hDC)
{
	...
	if(dwBorderColor != 0 || m_dwFocusBorderColor != 0)
	{
		...
		else
		{
			...
			else if(m_rcBorderSize.left > 0 || m_rcBorderSize.top > 0 || m_rcBorderSize.right > 0 || m_rcBorderSize.bottom > 0)
			{
				...
				if(m_rcBorderSize.left > 0){
					rcBorder		= m_rcItem;
                    if (m_rcBorderSize.left > 1 && PS_SOLID == m_nBorderStyle) {
                        rcBorder.right = rcBorder.left + m_rcBorderSize.left;
                        CRenderEngine::DrawColor(hDC, rcBorder, GetAdjustColor(dwBorderColor));
                    }
                    else {
				...
				if(m_rcBorderSize.top > 0){
					rcBorder		= m_rcItem;
                    if (m_rcBorderSize.top > 1 && PS_SOLID == m_nBorderStyle) {
				...
				if(m_rcBorderSize.right > 0){
                    rcBorder = m_rcItem;
                    if (m_rcBorderSize.right > 1 && PS_SOLID == m_nBorderStyle) {
				...
				if(m_rcBorderSize.bottom > 0){
                    ...
                    if (m_rcBorderSize.bottom > 1 && PS_SOLID == m_nBorderStyle) {
