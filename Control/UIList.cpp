...
//
    ...
    CListUI::CListUI() : m_pCallback(NULL), m_bScrollSelect(false), m_iCurSel(-1), m_iExpandedItem(-1), m_bRangeSelect(false), m_bMultiSelect(false), m_bMultiSelecting(false), m_iSelCount(0)
{
...
bool CListUI::Add(CControlUI* pControl, bool bRefresh)
{
    ...
    return m_pList->Add(pControl, bRefresh);
}
...
void CListUI::GetSelectedItems(std::vector<IListItemUI*>& item_lst)
{
    item_lst.clear();
    int count = GetCount();
...
bool CListUI::SelectItem(int iIndex, bool bTakeFocus)
{
    ...
    {
        if (m_iSelCount > 0) {
            std::vector<IListItemUI*> item_lst;
            GetSelectedItems(item_lst);
            for (std::vector<IListItemUI*>::iterator itor = item_lst.begin(); itor != item_lst.end(); ++itor)
            {
                if (*itor != NULL) (*itor)->Select(false);
            }
        }
        m_iCurSel = -1;
        m_iSelCount = 0;
...
//    if (!pControl->IsVisible()) return false;
    ...
    if (!bCtrl || !m_bMultiSelect)
    {
        if (m_iCurSel >= 0) {
            ...
            for (std::vector<IListItemUI*>::iterator itor = item_lst.begin(); itor != item_lst.end(); ++itor)
            {
                if (*itor != NULL && pListItem != *itor) (*itor)->Select(false);
            }
...
bool CListUI::SelectRange(int iIndex, bool bTakeFocus/* = false*/)
{
    if (m_bRangeSelect)
    {
        return true;
    ...
    IListItemUI* pListItem = static_cast<IListItemUI*>(pControl->GetInterface(_T("ListItem")));
    if (NULL == pListItem || !pListItem->Select(true)) {
        m_iSelCount = 0;
        m_iCurSel = -1;
        m_bRangeSelect = false;
        return false;
    ...
    while (i < iCount){
        ...
        if (pControl != NULL) {
            IListItemUI* pListItem = static_cast<IListItemUI*>(pControl->GetInterface(_T("ListItem")));
            if (pListItem != NULL) pListItem->Select(true);
        }
        if (i == iFirst || i == iLast){
            i++;
            break;
    ...
    m_iSelCount = abs(iLast - iFirst);
    m_bRangeSelect = false;
    return true;
}

void CListUI::SelectAll()
{
    if (!m_bMultiSelect)
    {
        return;
...
void CListElementUI::DrawItemBk(HDC hDC, const RECT& rcItem)
{
    ...
    TListInfoUI* pInfo = m_pOwner->GetListInfo();
    DWORD iBackColor = m_dwBackColor;
    if ((0 == iBackColor) && (!pInfo->bAlternateBk || m_iIndex % 2 == 0)) iBackColor = pInfo->dwBkColor;
...
void CListTextElementUI::DrawItemText(HDC hDC, const RECT& rcItem)
{
    ...
    for( int i = 0; i < pInfo->nColumns; i++ )
    {
        ...
        RECT rcPadding = pInfo->rcColumnTextPadding[i];
        if (0 == rcPadding.left && 0 == rcPadding.top && 0 == rcPadding.right && 0 == rcPadding.bottom)
        {
            rcPadding = pInfo->rcTextPadding;
        }
...
void CListTextElementUI::SetItemTextPadding(int iIndex, RECT& rcPadding)
{
    if (m_pOwner == NULL) return;
    TListInfoUI* pInfo = m_pOwner->GetListInfo();
    if (iIndex < 0 || iIndex >= pInfo->nColumns) return;
...
{
    m_pOwner = static_cast<IListOwnerUI*>(pOwner->GetInterface(_T("IListOwner")));
    IListUI* pList = static_cast<IListUI*>(pOwner->GetInterface(_T("IList")));
    if (NULL != pList)
    {
        m_pHeader = pList->GetHeader();
    }
...
bool CListContainerElementUI::Select(bool bSelect)
{
    ...
    if (m_pManager != NULL) {
        m_pManager->SendNotify(this, DUI_MSGTYPE_ITEMSELECT, bSelect, 0);
    }
